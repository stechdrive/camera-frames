...
- app version: `package.json` の `version` を正とする
...
- 連番は PNG / PSD を ZIP にまとめ、動画は PSD layer / hidden mask document を作らず PNG 相当の composite frame を WebCodecs で WebM に encode する
- WebM の frame timestamp / duration は clip fps を基準にし、frame ごとの render 待ち時間を動画尺に混ぜない
