...
スプラット編集 で加えた変更は複製にも引き継がれます（新しい独立アセットとして分離されます）。
...
- **削除** — 選択アセットを削除。undo 可能。関連する スプラット編集 の選択状態はクリアされる
- **複製** — 元アセットの直下に新アセットを複製。スプラット編集 の選択状態は引き継がれない

スプラット編集 の Separate / Duplicate から作られた新アセットは、元アセットの**直上**に挿入されます（[スプラット編集](09-per-splat-edit.md) で詳述）。
