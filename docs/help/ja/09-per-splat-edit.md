...
パッケージ保存で `LoD 事前計算` オプションを使って焼き込んだ LoD は、スプラット編集（`Delete` / `Separate` / `Duplicate` / `Transform` / `Color`）を行うと自動的に破棄されます。LoD ツリーはスプラットのインデックスに紐付いているため、内容が変わると整合性が取れなくなるためです。

編集後に高速読込みを取り戻したい場合は、次回のパッケージ保存で再度 `LoD 事前計算` を選んで焼き直してください。
