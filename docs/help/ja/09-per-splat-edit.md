...
スプラット の選択状態（どの スプラット が選ばれているか）は **runtime でのみ保持**され、project には保存されません。モードを抜けた時点や project save には含まれず、再度 スプラット編集 に入ると「選択なし」から再開します。

ただし undo / redo 用の history には含まれるので、編集セッション中の操作は巻き戻せます。

### 編集結果は persistent

Delete / Separate / Duplicate / Transform の結果は スプラット アセットの **source に永続化**されます。具体的には `asset.source`（PackedSplatSource）が再構築され、`.ssproj` の save にも含まれます。
...
モードに入ると ビューポート 内に専用 toolbar が現れます。scope（どの スプラット アセットを対象にするか）が自動で解決されます。scope が空なら「スプラット アセットがない」エラーで失敗。

## 3. Toolbar の構造

![スプラット編集 toolbar](../assets/screenshots/ja/per-スプラット-edit-toolbar.png)
...
| ボタン | ショートカット | 機能 |
|---|---|---|
| **Select All** | `Ctrl+A` | scope 内の全 スプラット を選択 |
...
| ボタン | 機能 | 有効条件 |
|---|---|---|
| **Delete**（danger style） | 選択 スプラット を削除 | 選択あり |
...
- ビューポート 上をクリック → シーンへレイキャスト、hit point を center にして Box を配置
...
![Box tool](../assets/screenshots/ja/per-スプラット-box-tool.png)
...
![Brush preview](../assets/screenshots/ja/per-スプラット-brush-preview.png)
...
gizmo の pivot（変形の中心）は、選択 スプラット の bounds の中央がデフォルトです。

### 6.4 Undo / Redo

Transform は history に `スプラット-edit.transform` として記録され、`Ctrl+Z` で巻き戻せます。
...
scope 内の**全アセットの全 スプラット** を選択に追加します。大量データでは時間がかかります。
...
1. 選択 スプラット を抽出
2. remainder（非選択 スプラット）で新しい source を生成
