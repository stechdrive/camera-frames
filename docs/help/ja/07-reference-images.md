...
- **背面** — 無限平面グリッドと 3D シーンの後ろ（下絵として透かす用途）
- **前面** — シーンの前（重ね合わせ用途）

プレビューは **背面下絵 → 無限平面グリッドと 3D シーン → 前面下絵** の順で合成されます。同じグループ内ではマネージャーの順序が使われ、フレームマスクは別レイヤーとして下絵プレビューより前面に表示されます。
...
読み込みの入口は下絵マネージャーセクションの + ボタン、ビューポートへのドラッグ & ドロップ、または `Ctrl+V` のクリップボード画像貼り付け。
...
テキスト入力中でない時に `Ctrl+V` を押すと、ブラウザがクリップボードから実体画像として渡した PNG / JPG / WebP 相当の bitmap を 1 アイテムとして追加します。URL 文字列や HTML 断片は読み込みません。
...
| キー | 動作 |
|---|---|
| `Ctrl+V` | クリップボード画像を下絵として読み込み（テキスト入力中を除く） |
| `R` | 下絵プレビュー表示切替 |
| `Shift+R` | 下絵編集モード切替 |
