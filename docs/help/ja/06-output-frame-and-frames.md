...
CAMERA_FRAMES の構図は、**用紙**（紙面そのもの）と **フレーム**（紙面上の矩形）の 2 つで成り立ちます。

- **用紙** — そのまま 書き出し の出力領域となる「紙面サイズ」
- **フレーム** — 紙面上に配置する矩形。1 つでも複数でも配置できる
...
フレーム は紙面上の矩形で、**構図の注目領域**を示します。単独でも複数でも配置できます。
...
フレーム の外側を暗くするマスク機能。「注目領域を視覚的に強調する」目的で使います。
...
フレーム 群の中心を繋ぐ path。shape = `軌道` の mask にも、書き出し時の軌道線にも使われます。
...
| 保存された 軌道 node vector | frame center 基準のままなので、自動で正しい位置へ |
