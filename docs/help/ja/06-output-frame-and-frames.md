...
| 操作 | 効果 |
|---|---|
| 枠線近傍をクリック | 用紙枠を選択。ドラッグになった場合は選択せず、カメラ操作を優先 |
| **8 リサイズハンドル** | 選択後、各辺 / 各角をドラッグして紙面サイズを変更（アンカー固定） |
| **4 パンエッジ** | 選択後、各辺中央をドラッグして紙面中心を平行移動 |
...
| 操作 | 効果 |
|---|---|
| 枠線近傍をクリック | 単独選択。ドラッグになった場合は選択せず、カメラ操作を優先 |
| `Shift` / `Ctrl` / `Meta` + 枠線近傍をクリック | 加算（切替） |
| 空きスペースをクリック | 選択解除 |
| `Ctrl+D` | 選択クリア |
...
|---|---|
| **移動** | 選択後、枠線をドラッグ |
| **回転** | 選択後、枠外周の回転ゾーンをドラッグ。15° スナップ |
| **リサイズ** | 選択後、8 つのリサイズハンドル（四隅 + 各辺中央）。`Alt` でフレーム自身のアンカーをピボットにする |
| **アンカー編集** | 選択後、フレーム上のアンカー点をドラッグ |
...
ブラウザ表示では、フレームマスクは下絵プレビューより前面、FRAME や編集ハンドルより背面に重なります。
...
軌道線とハンドルはフレーム選択中だけ表示されます。フレーム編集を確定して選択が解除されると、軌道線も非表示になります。
