...
- **ビューポート camera とは別** — ビューポート で自由に視点を動かしても shot は変わらない。shot は意図的に保存した構図だけが残る
- **常に perspective** — orthographic は ビューポート 側だけで、ショットカメラ には昇格しない

ショットカメラ の state は `.ssproj` の project document に保存され、書き出し は shot 単位で行われます。

## 2. カメラ一覧 セクション

インスペクター の Camera タブにあります。

![カメラ一覧](../assets/screenshots/ja/shot-camera-manager.png)

### 2.1 追加する

ボタン **[+]** で新規 ショットカメラ を追加。新規 shot は独立した初期値で作られ、現在の ビューポート 視点がコピーされることはありません。

### 2.2 複製する

ボタン **[Duplicate]** で active shot をまるごと複製します。pose / lens / フレーム / 用紙 も含めてコピーされ、独立編集できます。
...
- **→ Copy ビューポート to Shot** — ビューポート の現在視点を active shot に書き込む
- **← Copy Shot to ビューポート** — active shot の視点を ビューポート にコピー
...
| 操作 | orbit / pan / dolly で自由 | ショットカメラ 視点を直接動かす（shot に反映） |
| orthographic | 切替可能（ビューポート-only） | 常に perspective |
...
ビューポート で orthographic に切り替えたいときは、モード切替とは別に orthographic toggle が必要です（orthographic は ビューポート-only）。
