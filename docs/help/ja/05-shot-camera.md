...
## 4. Camera View と Viewport

ショットの構図確認に使う Camera View と、自由な作業視点の Viewport は別のカメラです。既定は Camera View 1 画面ですが、**2画面**にすると同じシーンを両方の視点から同時に確認できます。

| | Camera View | Viewport |
|---|---|---|
| 視点 | 選択中のショットカメラ | エディタ用の作業カメラ |
| 操作 | ショットカメラ視点を直接動かす（ショットカメラに反映） | オービット / パン / ドリーで自由 |
| 正投影 | 常に透視投影 | 切替可能（Viewport のみ） |
| 用紙枠 | **表示される** | 表示されない |
| フレームマスク | 設定に応じて表示 | 表示されない |
| 下絵 | プレビュー可・編集可 | プレビュー可 |

2 画面を同時表示している間、下絵は Camera View 側に合成されます。Camera View を閉じて Viewport だけにすると、従来どおり Viewport 上で下絵をプレビュー・編集できます。

![Camera View で用紙枠が表示された状態](../assets/screenshots/ja/camera-mode-render-box.png)

### 4.1 2 つの view の意味

- **Camera View** — 書き出しされる構図を**そのまま**見ている視点。ショットカメラを選び直すと、そのカメラへ切り替わる
- **Viewport** — シーンの様子を自由に見る作業視点。ショットカメラの選択や操作とは別に保持される

どちらも同じシーンを描画しますが、**目的が違う**という位置付けです。

### 4.2 表示と操作対象

- **2画面**ボタンで Camera View と Viewport を同時表示
- pane header の閉じるボタンで片方を閉じ、残った view を全画面表示
- 2画面時は click した pane が操作対象
- divider は pointer drag または keyboard で移動
- **パイメニュー**（中ボタンドラッグ）の **カメラ/ビューポート** 項目で、表示または操作対象の view を切替

Viewport で正投影に切り替えたいときは、view の表示切替とは別に正投影トグルが必要です（正投影は Viewport のみ）。2画面の表示状態や比率は端末 local の UI 設定で、`.ssproj` や undo / redo には含まれません。

## 5. 視点の直接操作

Camera View / Viewport 共通で、操作対象になっている pane に次のマウス操作が使えます。
...
Camera View が操作対象の時だけ有効。パイメニュー経由または内部コマンドから起動します。
