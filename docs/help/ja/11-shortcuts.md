...
| キーをクリック | キーを選択し、そのフレームへ移動 |
| `Shift` / `Ctrl` / `Meta` + キーをクリック | キー選択を追加 / 解除 |
| 選択キーをドラッグ | 選択中のキーをまとめて移動 |
| `I` | 現在のキー対象にキーを追加 |
| `,` | 前のキーへ移動 |
| `.` | 次のキーへ移動 |
| `Ctrl` / `Meta` + `C` | 選択キーをコピー |
| `Ctrl` / `Meta` + `V` | コピーしたキーを現在フレームへ貼り付け |
| `Delete` / `Backspace` | 選択キーを削除 |
| `Escape` | キー選択を解除 |

`キーを追加` を実行すると、キーが作成されたカメラ / シーンアセット行の Auto Key が ON になります。Auto Key はタイムラインのトラック行にあるキーアイコンで対象ごとに切り替えます。ON の対象だけが、現在フレームで編集した時にそのフレームのキーを更新します。

選択キーはタイムライン上で `Linear` / `Hold` の補間を切り替えられます。複数フレームのキーを選択して `Scale` を適用すると、最初の選択フレームを基準にキー間隔を伸縮します。Graph Editor は現時点ではありません。
