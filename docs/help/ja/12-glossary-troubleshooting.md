...
- **フレーム** — アニメ撮影で言う**撮影フレーム**。紙面内のどこを撮るかを矩形で示す。1 ショット につき最大 20 個。**1 つなら静止カット**、**複数ならパン・TU/TB・ズームなどのカメラワーク指示**として使う。[→ 6](06-output-frame-and-frames.md)
...
これは意図した動作です。1 → 2 フレーム に増えた瞬間、mask shape が `bounds` なら `trajectory` に自動昇格します（load 時を除く）。trajectoryExportSource も自動で最適な corner を選びます。

#### Q. 軌道 handle が ビューポート に出ない

軌道編集トグル が OFF か、軌道モード が `line` になっている可能性があります。spline mode にして、フレーム セクションの 軌道編集 を ON にしてください。
