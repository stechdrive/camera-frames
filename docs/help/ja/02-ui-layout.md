...
- プレビュー品質 — 3DGS が重いときに、ビューポート表示の軽さと細部の見やすさを調整する端末ごとの local 設定
...
HTTP(S) URL を入力して Load ボタンで読み込む欄。外部ホストの シーンアセット を取り込めます。
...
- **New Frame** — フレーム を追加
- **フレームマスク 設定** — マスクモード（off / all / selected）、不透明度、形状（bounds / 軌道）、軌道 書き出し元
...
| **Camera** | {{icon:camera-dslr}} | ショットカメラ の管理、カメラプロパティ、用紙 |
...
- **カメラ一覧** — ショットカメラ 一覧、追加、選択、削除、並び替え、書き出し ファイル名編集
- **カメラプロパティ** — アクティブな ショットカメラ の pose、lens、clipping、ロールロック
...
- **下絵プリセット** — プリセット 作成・切替・削除、現在の ショットカメラ への紐付け
- **下絵マネージャー** — アクティブな プリセット 内の アイテム 一覧、表示切替、前後グループ、書き出し対象切替
- **下絵プロパティ** — 選択アイテム の位置・回転・拡縮、複数選択時の一括変換

詳しくは [リファレンス画像](07-reference-images.md)。

#### 書き出し タブ

- **Output** — 書き出し実行、対象（`current` / `all` / `selected`）、フォーマット（PNG / PSD）
- **書き出し設定** — PSD レイヤー オプション、グリッド、下絵 参加有無、書き出しファイル名
