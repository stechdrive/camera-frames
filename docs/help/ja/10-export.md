...
**Download Output** ボタンを押すと、UI が一時的に操作できなくなり、進捗オーバーレイ が表示されて書き出しが始まります。

### 3.2 フェーズ

進捗オーバーレイ には現在の処理フェーズが表示されます。
...
|---|---|
| **準備** | 書き出し可能かチェック、前処理 |
| **メイン描画** | ショットカメラ からのメインレンダリング |
| **ガイド** | グリッド と水平線（グリッド オーバーレイ ON 時） |
| **マスク** | フレームマスク（マスクあり時） |
| **PSD ベース合成** | PSD 書き出し時の下地合成 |
| **モデルレイヤー** | モデルアセット をレイヤー化（オプション ON 時） |
| **スプラットレイヤー** | スプラット アセットをレイヤー化（オプション ON 時） |
| **下絵** | 下絵 画像のレイヤー化（下絵 を含める ON 時） |
| **書き込み** | ファイルに書き出し |
...
フレームマスク が出ていた場合、その領域を示す マスクレイヤー が**非表示かつ不透明度 0.8** で追加されます。必要なときだけ Photoshop 側で表示を ON にして使います。
...
スプラット アセットがある場合、書き出しの直前に表示品質を安定させるための短いウォームアップ処理（最大 1.5 秒ほど）が自動で入ります。

### 7.2 アクティブな ショットカメラ

`current` 対象では、アクティブな ショットカメラ が無いと 書き出し できません（エラー表示）。

### 7.3 下絵 の可用性

下絵 を含める が ON でも、画像ファイルが取得できない アイテム はレイヤーに出ません（該当ファイルはスキップ）。
